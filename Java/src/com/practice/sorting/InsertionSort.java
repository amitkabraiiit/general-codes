...
public class InsertionSort{
	 ...
	 * 
	 * Suppose the array length is n. The outer loop runs roughly n times, and the inner 
	 * loop on average runs n/2 times. The total time is about t(n) = n * (n/2) = O(n^2). 
	...
	public static void insertionSort(int[] ar)
	{
		...
		for (int i = 1; i < ar.length; i++) {
			for(int j = i ; j > 0 ; j--){ // suppose till now sorted array is 1 2 7 9 and now we got 
			    // 3 then we need to put 3 after 2, so we first swap 3 with 9 then with 3 with 7 
				// and then 3 gets its position.
				if(ar[j] < ar[j-1]){
					temp = ar[j];
					ar[j] = ar[j-1];
					ar[j-1] = temp;
